 ...
 *
 *   {
 *     pureName:  //the filename (not include the file path) (longxiuping add)
...
var MediaDB = (function() {
    ...
    function initDeviceStorage() {
      ...
      function changeHandler(e) {
        switch (e.reason) {
          case 'modified':
          case 'deleted':
            // tcl_longxiuping modified for bug 712898
            setTimeout(function() {
              fileChangeHandler.bind(this);
            }, 3000);
            return;
  ...
  MediaDB.prototype = {
    ...
    //    pureName: // the filename not contain path tcl_longxiuping add
    //    name: // the filename
    //    type: // the file type
    //    size: // the file size
    //    date: // file mod time
    ...
    enumerate: function enumerate(key, range, direction, callback) {
      console.log('lxp:: mediadb enumerate key = ' + key);
      if (this.state !== MediaDB.READY)
        throw Error('MediaDB is not ready. State: ' + this.state);
      ...
      cursorRequest.onsuccess = function() {
        ...
        if (cursor) {
          try {
            ...
            //tcl_longxiuping delete for list all file name
            callback(cursor.value);
          }
          catch (e) {
  ...
  function processQueue(media) {
    ...
    function deleteFiles() {
      ...
      function deleteNextFile() {
        ...
        getRequest.onerror = function(event) {
          // This probably means that the file wasn't in the device storage yet
          console.warn('MediaDB: Get unknown file in deleteRecord:',
            filename, getRequest.error);
          deleteNextFile();
        };
        getRequest.onsuccess = function(event) {
          ...
          // delete that, too.
          if (fileinfo && fileinfo.metadata && fileinfo.metadata.preview &&
    ...
    function parseMetadata(file, filename) {
      ...
      // tcl_longxiuping add for name sort.
      var index = filename.lastIndexOf('/');
      var pureName = filename.substring(index + 1);

      // Basic information about the file
      var fileinfo = {
        pureName: pureName, //tcl_longxiuping add for name sort
    ...
    function storeRecord(fileinfo) {
      if (media.mediaType == 'music' &&
        fileinfo.fail != undefined &&
        fileinfo.fail == true) {
        console.log('lxp:: storeRecord music fileinfo = ' +
          JSON.stringify(fileinfo));
        next();
        return;
      }

      if (media.mediaType == 'videos' &&
        fileinfo.metadata.isVideo == false) {
        console.log('lxp:: storeRecord videos fileinfo = ' +
          JSON.stringify(fileinfo));
        next();
        return;
      }
      ...
      if (media.details.firstscan) {
        ...
        //if (!fileinfo.fail) { //tcl_longxiuping delete for list all file name
  ...
  function dispatchEvent(media, type, detail) {
    ...
    //console.log('lxp:: handler = ' + handler);
